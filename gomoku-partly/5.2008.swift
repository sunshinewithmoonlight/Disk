...
//所有可能传入的变量在这里，分为全局变量/传入变量，函数内变量，小变量/临时变量
...
func countWinTimes (times:Int, obj:Character="m", listmIndex_input:[Int] = listmIndex_input, listnIndex_input:[Int] = listnIndex_input, listCB:[Int] = listCB, listCB_show:[[Character]] = listCB_show, aroundIndexes:[Int] = aroundIndexes) {
    var countWinTimes_1 = Array(repeating: 0, count: 361)
    var countWinTimes_next = [Int]()
    if obj == "m" {
        countWinTimes_next.append(0);
        } else if obj == "n" {
        countWinTimes_next.append(1);
        } else {
        print("\(obj) goes wrong.");
        return
    }
    countWinTimes_next.append(-1)
    var countWinTimes_2 = 0
    var countWinTimes_3 = 0
    var countWinTimes_4 = 0.0
    var countWinTimes_next_aroundIndexes = aroundIndexes
    print(listmIndex_input, listnIndex_input)

    countWinTimes_next_aroundIndexes = Array(Set(countWinTimes_next_aroundIndexes))
    var i = 0
    while (i < countWinTimes_next_aroundIndexes.count && i > -1) {
        if (countWinTimes_next_aroundIndexes[i] < 0 || countWinTimes_next_aroundIndexes[i] > 360 || listmIndex_input.contains(countWinTimes_next_aroundIndexes[i]) || listnIndex_input.contains(countWinTimes_next_aroundIndexes[i])) {
            countWinTimes_next_aroundIndexes.remove(at:i)
            i -= 1
        }
        i += 1
    }

    let countWinTimes_6 = times*countWinTimes_next_aroundIndexes.count

    if listmIndex_input.count == listnIndex_input.count {
        for times_ in 0..<times {
            print("    "+String(countWinTimes_6/times*countWinTimes_2)+"    /    "+String(countWinTimes_6)+"       "+String(times_)+"  done              "+"\r")
            for countWinTimes_next_aroundIndexes_ in countWinTimes_next_aroundIndexes {
                countWinTimes_next[1] = countWinTimes_next_aroundIndexes_
                //(countWinTimes_3, countWinTimes_6) = perRound_N(perRound_mode:"func_e_countWinTimes", perRound_mnIndex:listmIndex_input, perRound_mnIndex:listnIndex_input, perRound_next:countWinTimes_next)
                if countWinTimes_6 == 1 {
                    countWinTimes_1[countWinTimes_next[1]] += countWinTimes_3
                } else {
                    countWinTimes_1[countWinTimes_next[1]] = -1
                }
            }
            countWinTimes_2 += 1
        }
    } else if listmIndex_input.count == listnIndex_input.count+1 {
        for times_ in 0..<times {
            print("    "+String(countWinTimes_6/times*countWinTimes_2)+"    /    "+String(countWinTimes_6)+"       "+String(times_)+"  done              "+"\r")
            for countWinTimes_next_aroundIndexes_ in countWinTimes_next_aroundIndexes {
                countWinTimes_next[1] = countWinTimes_next_aroundIndexes_
                (countWinTimes_3, countWinTimes_6) = perRound(perRound_mode:"func_e_countWinTimes", perRound_mnIndex:listmIndex_input, perRound_mnIndex:listnIndex_input, perRound_next:countWinTimes_next)
                if countWinTimes_6 == 1 {
                    countWinTimes_1[countWinTimes_next[1]] += countWinTimes_3
                } else {
                    countWinTimes_1[countWinTimes_next[1]] = -1
    ...
    print("    "+String(countWinTimes_6)+"    /    "+String(countWinTimes_6)+"       "+String(times)+"  done"+"\r")
    print(countWinTimes_1)
    for countWinTimes_1_ in countWinTimes_1 {
        if countWinTimes_1_ != -1 {
            countWinTimes_4 += Double(countWinTimes_1_)
        }
    }  
    print(String(countWinTimes_4/Double(countWinTimes_6)))
    print()
}
